...
def parseChanData(chan, data):
    ...
    if (chan % 2) > 0:
        I = data[int(1024 + ((chan - 1) / 2))]
        Q = data[int(1536 + ((chan - 1) / 2))]
    else:
        I = data[int(0 + (chan/2))]
        Q = data[int(512 + (chan/2))]
...
def writer(q, filename, start_chan, end_chan): #Haven't tested recently, but as 
    ...
    nfo_I = [filename + "/I_" + str(z) for z in chan_range]
    nfo_Q = [filename + "/Q_" + str(z) for z in chan_range]
    ...
    fo_I = [open(z, "ab") for z in nfo_I]
    fo_Q = [open(z, "ab") for z in nfo_Q]
    ...
    while streaming:
        ...
        if q_data is not None:
            ...
            for idx, chan in enumerate(range(start_chan, end_chan + 1)):
                ...
                I = data[int((chan % 2) * 1024 + (chan / 2))]
                Q = data[int(512 + (chan % 2) * 1024 + (chan / 2))]
...
class roachDownlink(object):
    ...
    def saveSweepData(self, Navg, savepath, lo_freq, Nchan, skip_packets = None):
        ...
        while count < Navg + skip_packets:
            try:
                ...
                if len(channels) % 2 > 0:
                    if len(I_odd) > 0:
                        I = np.hstack(list(zip(I_even[:len(I_odd)], I_odd)))
                        Q = np.hstack(list(zip(Q_even[:len(Q_odd)], Q_odd)))
                    ...
                    I = np.hstack(list(zip(I_even, I_odd)))
                    Q = np.hstack(list(zip(Q_even, Q_odd)))
                I_buffer[count] = I
                Q_buffer[count] = Q
    ...
    def saveDirfile_chanRange(self, time_interval, stage_coords = False):
        ...
        start_chan = eval(input("Start chan # ? "))
        end_chan = eval(input("End chan # ? "))
        chan_range = list(range(start_chan, end_chan + 1))
        data_path = self.gc[np.where(self.gc == 'DIRFILE_SAVEPATH')[0][0]][1]
        sub_folder = input("Insert subfolder name (e.g. single_tone): ")
        Npackets = np.ceil(time_interval * self.data_rate)
        ...
        d.close()
        d = gd.dirfile(filename,gd.RDWR|gd.UNENCODED)
        nfo_phase = [filename + "/chP_" + str(z) for z in chan_range]
        fo_phase = [open(z, "ab") for z in nfo_phase]
        fo_time = open(filename + "/time", "ab")
        fo_count = open(filename + "/packet_count", "ab")
        count = 0
        while count < Npackets:
