...
#           self.fpga.write_int(self.regs[np.where(self.regs == 'fir_prefix_reg')[0][0]][1] + str(i),coeff)
    ...
    def get_transfunc(self):
        ...
        return transfunc
    '''
    def freqComb(self, freqs, samp_freq, resolution, random_phase = True, DAC_LUT = True, apply_transfunc = True, **kwargs):
    ...
    def dirfile_all_chan(self, time_interval):
        ...
        channels = list(range(nchannel))
        data_path = "./data"
        sub_folder_1 = "meas"
        sub_folder_2 = input("Insert subfolder name (e.g. single_tone): ")
...
#        self.fpga.write_int(self.regs[np.where(self.regs == 'pps_start_reg')[0][0]][1], 1)
        ...
        nfo_phase = [filename + "/chP_" + str(z) for z in range(nchannel)]
    ...
    def plotADC(self):
        ...
        while count < stop:    
            ...
            adc /= (2**15)
            adc *= 550.
            I = np.hstack(list(zip(adc[0::4],adc[2::4])))
            Q = np.hstack(list(zip(adc[1::4],adc[3::4])))
    ...
    def plotFFT(self):
        ...
        while(count < stop):
            ...
            mags = np.hstack(list(zip(mag0, mag1)))
            mag1 = 20*np.log10(mag1)
            fft_mags = np.hstack(list(zip(mag0,mag1)))
