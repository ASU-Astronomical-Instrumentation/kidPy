    ...
    while 1:
        ...
        if opt == 9:
            if not fpga:
                print("\nROACH link is down")
                break
            time_interval = eval(input('\nNumber of seconds to stream? ' ))
            chan = eval(input('chan = ? '))
            try:
        ...
        if opt == 14:
            if not fpga:
                print("\nROACH link is down")
                break
            chan = eval(input('Channel number = ? '))
            time_interval = eval(input('Time interval (s) ? '))
            try:
...
def main():
    ...
    while 1:
        try:
            ...
            upload_status= main_opt(None, None, None, None, upload_status)
        except TypeError:
            pass
    return 
